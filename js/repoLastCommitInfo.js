      ...
      fetch(`https://api.github.com/repos/readytotest/${repo.name}/commits`)
        ...
        .then(commits => {
          ...
          repoTimestamper.insertAdjacentHTML(
            'beforeend',
            `<a href="${repo.html_url}" target="_blank" rel="noopener noreferrer">${repo.name}</a>:<br>${new Date(repo.pushed_at)}<br>By:${lastCommit.commit.committer.name}:<br>"${lastCommitMsg}"<br>`
          );
        })
        .catch(error => {
