      ...
      fetch(`https://api.github.com/repos/readytotest/${repo.name}/commits`)
        ...
        .then(commits => {
          ...
          repoTimestamper.insertAdjacentHTML(
            'beforeend',
            `<a href="${repo.html_url}" target="_blank" rel="noopener noreferrer">${repo.name}</a>:<br>Topics: ${repo.topics}<br>Date: ${new Date(repo.pushed_at).toLocaleString('en-US', { timeZone: 'America/Los_Angeles' })} Pacific<br>git commit -m: "${lastCommitMsg}"<br>`
          );
        })
        .catch(error => {
